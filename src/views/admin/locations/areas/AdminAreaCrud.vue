...
export default {
  ...
  data () {
    return {
      ...
      modalArea: false,
      validationArea: yup.object().shape({
  ...
  methods: {
    ...
    async validateArea() {
      this.$refs.formArea.validate({ silent: true }).then(
        async validArea => {
          if (!validArea) {
            return overlays.invalid()
          }

          const createOrUpdate = this.state.editing ? SAdminAreas.put : SAdminAreas.post

          overlays.confirm({
            confirmButtonText: this.state.editing ? 'Update' : 'Create',
            body: `${this.state.editing ? 'Update' : 'Create'} Area?`,
            preConfirm: async () => {
              return createOrUpdate(this.formArea).then(
                ({ data: { message, area }}) => {
                ...
                ({ message }) => {
                  if (message && message.errors) {
                    ...
                    this.$refs.formArea.setErrors(validationError)
                  }
                }
              ).finally(() => {
