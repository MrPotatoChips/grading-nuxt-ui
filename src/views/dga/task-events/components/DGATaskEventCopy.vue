<template>
  ...
  >
    <template #header>
      <UUser
        :name="`${card.task?.assigned?.first_name} ${card.task?.assigned?.last_name}`"
        :description="dateFormatters.toDateTimeShort(card.created_at)"
        :avatar="{
    ...
    <div class="flex flex-col gap-2 mb-2">
      <div>
        <div class="text-xs">
          <b>{{ card.activity?.activity_name }}</b> for <b>{{ card.crop?.crop_name }}</b>, last <b>{{ dateFormatters.toDateShort(card.event_date) }}</b>
        </div>
        <div class="text-xs">
          Held at <b>{{ card.barangay?.barangay_name }}</b>, <b>{{ card.municipality?.municipality_name }}</b>, <b>{{ card.province?.province_name }}</b>
        </div>
      </div>
